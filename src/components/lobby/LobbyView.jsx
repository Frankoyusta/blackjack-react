  ...
  return (
    <div className="lobby-container max-w-4xl mx-auto p-6 bg-white rounded-lg shadow-md">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-3xl font-bold text-black">Blackjack Online</h1>
      ...
      <div className="mb-8">
        <h2 className="text-2xl font-bold mb-4 text-black">Crear Nueva Mesa</h2>
        <form onSubmit={handleCreateTable} className="flex gap-2">
          <input 
            type="text" 
            value={newTableName} 
            onChange={(e) => setNewTableName(e.target.value)}
            className="flex-grow px-3 py-2 border rounded-md text-black"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {tables.map(table => (
              <div key={table.id} className="p-4 border rounded-lg bg-gray-50">
                <h3 className="font-bold text-lg mb-2 text-black">{table.name}</h3>
                <p className="mb-2 text-black">Jugadores: {table.players}/{table.maxPlayers}</p>
                <p className="mb-3 text-black">Estado: {
